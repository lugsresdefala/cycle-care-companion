...
async function main() {
  ...
  try {
    ...
    const heartbeat = setInterval(() => { console.log(`[hb] captured=${frames.length}`); }, 5000);
  ...
  console.log(`Captured ${frames.length} frames over ${realDuration.toFixed(2)}s`);
  ...
  console.log('Wrote concat file:', CONCAT_FILE);
}
