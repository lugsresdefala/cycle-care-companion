/**
 * Server-side INTERGROWTH-21st growth assessment.
 * Kept server-side so premium growth curve analysis cannot be executed
 * from client bundles without authentication and token deduction.
 */
...
const VALID_PARAMS = new Set<string>(["efw", "hc", "ac", "fl", "bpd"]);

export function isValidGrowthParam(param: string): param is GrowthParameter {
  return VALID_PARAMS.has(param);
}
