/**
 * Server-side Doppler Velocimetry Calculators
 * Kept server-side to prevent client-bundle extraction of premium logic.
 * References:
 * - Acharya G et al. (2005) — Umbilical Artery PI
 * - Ebbing C et al. (2007) — MCA PI
 * - Gómez O et al. (2008) — Uterine Artery PI
 * - Baschat AA & Gembruch U (2003) — CPR
 * - Kessler J et al. (2006) / DeVore GR (2021) — Ductus Venosus PIV
 */
...
const UA_PI_REFS: Record<number, DopplerRefs> = {
...
const MCA_PI_REFS: Record<number, DopplerRefs> = {
...
const UTA_PI_REFS: Record<number, DopplerRefs> = {
...
const CPR_REFS: Record<number, DopplerRefs> = {
...
const DV_PIV_REFS: Record<number, DopplerRefs> = {
...
function getClosestGA(table: Record<number, DopplerRefs>, ga: number): number {
...
export function evaluateUmbilicalArteryPI(pi: number, gaWeeks: number): DopplerResult & { refs: DopplerRefs } {
  ...
  return { value: pi, percentile, interpretation, severity, refs };
}
...
export function evaluateMCAPI(pi: number, gaWeeks: number): DopplerResult & { refs: DopplerRefs } {
...
export function evaluateUterineArteryPI(pi: number, gaWeeks: number, bilateralNotch: boolean): DopplerResult & { refs: DopplerRefs } {
...
export function calculateCPR(mcaPI: number, uaPI: number, gaWeeks: number): CPRResult & { refs: DopplerRefs } {
  if (uaPI <= 0) {
    const refs = CPR_REFS[getClosestGA(CPR_REFS, gaWeeks)];
    return { cpr: 0, mcaPI, uaPI, percentile: "N/A", interpretation: "IP da artéria umbilical deve ser maior que zero.", severity: "critical", refs };
  }
  ...
  return { cpr, mcaPI, uaPI, percentile, interpretation, severity, refs };
}

export function evaluateDuctusVenosusPIV(piv: number, gaWeeks: number): DopplerResult & { refs: DopplerRefs } {
  ...
  return { value: piv, percentile, interpretation, severity, refs };
}
