/**
 * Risk Calculators for Obstetric Screening — SERVER-SIDE ONLY
 *
 * These implementations live on the server so that premium formula logic
...
function ntToMoM(ntMm: number, crl: number): number {
  return ntMm / ntExpectedMedian(crl);
}
...
function ntLikelihoodRatio(ntMm: number, crl: number, trisomy: "T21" | "T18" | "T13"): number {
  ...
  const pdfUnaffected = Math.exp(-0.5 * ((delta) / unaffectedSD) ** 2) / unaffectedSD;
  const pdfAffected = Math.exp(-0.5 * ((delta - affMean) / affSD) ** 2) / affSD;
  return Math.max(0.01, Math.min(100, pdfAffected / pdfUnaffected));
}
...
export function calculateTrisomyRisk(input: TrisomyInput): TrisomyResult {
  ...
  for (let i = 0; i < trisomies.length; i++) {
    ...
    combinedLR *= additionalMarkerLR(input.nasalBone, input.ductusPIAbnormal, input.tricuspidRegurg, tri);
    const priorOdds = 1 / bgRisks[i];
    const posteriorOdds = priorOdds * combinedLR;
    adjustedRisks.push(Math.max(2, Math.round(1 / posteriorOdds)));
  }
...
export function calculatePreeclampsiaRisk(input: PreeclampsiaInput): PreeclampsiaResult {
  if (input.height <= 0) throw new Error("Altura deve ser maior que zero.");
  ...
  if (input.ethnicityAfroCaribbean) { logOddsPreterm += 0.60; logOddsEarly += 0.75; logOddsAny += 0.45; }
  if (input.nulliparous) { logOddsPreterm += 0.70; logOddsEarly += 0.65; logOddsAny += 0.60; }
  if (input.conceptionIVF) { logOddsPreterm += 0.45; logOddsEarly += 0.55; logOddsAny += 0.30; }
  if (input.chronicHypertension) { logOddsPreterm += 1.80; logOddsEarly += 2.10; logOddsAny += 1.40; }
  if (input.diabetesType1or2) { logOddsPreterm += 0.85; logOddsEarly += 0.95; logOddsAny += 0.55; }
  if (input.lupusSLEorAPS) { logOddsPreterm += 1.20; logOddsEarly += 1.50; logOddsAny += 0.90; }
  if (input.previousPE) { logOddsPreterm += 1.65; logOddsEarly += 1.85; logOddsAny += 1.20; }
  if (input.familyHistoryPE) { logOddsPreterm += 0.40; logOddsEarly += 0.45; logOddsAny += 0.35; }

  if (input.meanArterialPressure !== null) {
    const mapLogMoM = Math.log(input.meanArterialPressure / 85);
  ...
  if (input.uterineArteryMeanPI !== null) {
    const piLogMoM = Math.log(input.uterineArteryMeanPI / 1.65);
