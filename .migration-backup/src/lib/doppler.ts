/**
 * Doppler Velocimetry Calculators
 * References:
 * - Acharya G et al. (2005) — Umbilical Artery PI reference ranges
 * - Ebbing C et al. (2007) — MCA PI reference ranges
 * - Gómez O et al. (2008) — Uterine Artery PI reference ranges
 * - Baschat AA & Gembruch U (2003) — CPR (Cerebroplacental Ratio)
 */
...
// ---- Umbilical Artery PI Reference Ranges (Acharya 2005) ----
// Approximation: mean PI decreases with GA
...
// ---- MCA PI Reference Ranges (Ebbing 2007) ----
...
// ---- Uterine Artery PI Reference Ranges (Gómez 2008) ----
...
// ---- CPR Reference (Baschat & Gembruch 2003) ----
...
function classifyPI(
  ...
  if (highIsAbnormal) {
    ...
    if (value > refs.p50 + (refs.p95 - refs.p50) * 0.6)
      return { percentile: "p75–p95", severity: "warning" };
    if (value < refs.p5) return { percentile: "< p5", severity: "warning" };
    return { percentile: "p5–p95 (normal)", severity: "normal" };
  } else {
    // Low is abnormal (e.g., MCA, CPR)
    if (value < refs.p5) return { percentile: "< p5", severity: "critical" };
    if (value < refs.p50 - (refs.p50 - refs.p5) * 0.6)
      return { percentile: "p5–p25", severity: "warning" };
...
export function evaluateUmbilicalArteryPI(pi: number, gaWeeks: number): DopplerResult {
  ...
  if (severity === "critical")
    interpretation = "IP significativamente elevado — avaliar insuficiência placentária. Considerar monitorização com cardiotocografia e perfil biofísico fetal.";
...
export function evaluateUmbilicalArterySDRatio(sd: number, gaWeeks: number): DopplerResult {
  ...
  if (sd >= threshold && sd < threshold + 1) {
    severity = "warning";
    percentile = "Limítrofe";
    interpretation = `Relação S/D discretamente elevada para ${gaWeeks} semanas.`;
  } else if (sd >= threshold + 1) {
    severity = "critical";
    percentile = "Elevado";
...
export function evaluateMCAPI(pi: number, gaWeeks: number): DopplerResult {
  ...
  if (severity === "warning")
    interpretation = "IP da ACM discretamente reduzido — possível início de vasodilatação cerebral compensatória.";
  if (severity === "critical")
    interpretation = "IP da ACM significativamente reduzido — brain-sparing effect. Avaliar centralização fetal e programar monitorização intensiva.";
...
export function calculateCPR(mcaPI: number, uaPI: number, gaWeeks: number): CPRResult {
  ...
  if (severity === "warning")
    interpretation = "RCP discretamente reduzida — possível início de centralização fetal. Repetir em 7 dias.";
  if (severity === "critical")
    interpretation = "RCP significativamente reduzida (< p5) — centralização fetal. Avaliar vitalidade fetal e considerar antecipação do parto conforme idade gestacional.";
...
export function getMCAPiRefs(gaWeeks: number) {
  return MCA_PI_REFS[getClosestGA(MCA_PI_REFS, gaWeeks)];
}
export function getUtAPiRefs(gaWeeks: number) {
  return UTA_PI_REFS[getClosestGA(UTA_PI_REFS, gaWeeks)];
}
