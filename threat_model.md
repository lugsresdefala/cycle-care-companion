...
- **Subscription-lifecycle caveat:** free-trial provisioning, Stripe checkout/webhook reconciliation, and token spending all converge on `user_subscriptions`. Future scans should re-check this area for multi-row entitlement bugs, bootstrap concurrency races, and token resets caused by event ordering changes.
